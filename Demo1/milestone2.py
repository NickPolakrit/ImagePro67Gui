...
while(showLive):
    ...
    # ret, frame = cap.read()
    frame = cv2.imread("crop_card 2.png")
    ...
    # Denoise = cv2.fastNlMeansDenoisingMulti(frame, 2, 5, None, 4, 7, 35)
    # unsharpV = cv2.addWeighted(
    #     frame, 1.5, Mblurred, -0.5, 0, frame)
    ...
    # result_resize = rescale_result(frame)
    # if not ret:
    ...
    # cv2.putText(frame, ":" + rX, (rX - 25, rY - 25),
    ...
    try:
        ...
        cv2.circle(frame, (yX, yY), 5, (0, 255, 255), -1)
    except:
        pass
    ...
    try:
        ...
        cv2.circle(frame, (blX, blY), 5, (0, 0, 0), -1)
    except:
        pass
    ...
    # Draw Text
    cv2.rectangle(frame, (0, 0), (100, 110), (0, 0, 0), -1)
    cv2.putText(frame, 'Red    :' + str(countRed),
                (10, 20),                  # bottomLeftCornerOfText
                cv2.FONT_HERSHEY_SIMPLEX,  # font
                0.5,                      # fontScale
                (0, 0, 255),            # fontColor
                1)                        # lineType
    cv2.putText(frame, 'Yellow :' + str(countYellow),
                (10, 40),                  # bottomLeftCornerOfText
                cv2.FONT_HERSHEY_SIMPLEX,  # font
                0.5,                      # fontScale
                (0, 255, 255),            # fontColor
                1)
    cv2.putText(frame, 'Blue   :' + str(countBlue),
                (10, 60),                  # bottomLeftCornerOfText
                cv2.FONT_HERSHEY_SIMPLEX,  # font
                0.5,                      # fontScale
                (255, 238, 0),            # fontColor
                1)
    cv2.putText(frame, 'Green  :' + str(countGreen),
                (10, 80),                  # bottomLeftCornerOfText
                cv2.FONT_HERSHEY_SIMPLEX,  # font
                0.5,                      # fontScale
                (0, 255, 0),            # fontColor
                1)
    cv2.putText(frame, 'Black  :' + str(countBlack),
